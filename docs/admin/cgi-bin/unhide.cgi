...
sub hide_abuse {
	...
	if ($commentid  ne '') { 
		...
		$dbh->do("update comments set visible=1 where postid=$postid_q and site='$site_name' and commentid=$commentid_q");
		$dbh->do("update posts set commentcount=commentcount+1 where postid=$postid_q and site='$site_name' ");
	} else {
		$dbh->do("update posts set hidden=0 where postid=$postid_q and site='$site_name' ");
	}
}
